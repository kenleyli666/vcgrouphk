  ...
  if (window.innerWidth <= 1024) {
    ...
    if (scrollTop - lastScrollTop >= 0 * vhInPx) {
      setTimeout(function () {
        ...
        Burger.style.color = "var(--text2-color)";
        // CtrlBtns.style.background = "var(--text1-color)";
...
// navtwo contral
document.querySelectorAll('.navbar-two a[href^="#"]').forEach((link) => {
  link.addEventListener("click", () => {
    document.querySelector(".navbar-two").classList.remove("active-navbar-two");
    body.classList.toggle("hide-scrollbar");
    if (body.style.overflow === "hidden") {
      body.style.overflow = "auto";
    } else {
